...
object PatentFilters {
  val filters:Seq[Filter] = Seq(
    ...
    new Filter(x => (x \ "us-bibliographic-data-grant" \ "classification-national" \ "main-classification"), s => Set("1", "2", "3", "4", "5", "6", "7", "8", "9","D").contains(s.text.trim.head.toString), "uspto_in_domain"), //todo this is rejecting too many things to start with
